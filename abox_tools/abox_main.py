...
from PyQt5.QtWidgets import QApplication, QMainWindow, QDialog, QMessageBox, QStatusBar, QFileDialog
...
class DSUtils():
    ...
    def FromSubdataset(dctFiles):
        ...
        lstRet = []
        for k in k2:
            xyxys = [[x[0], x[1], x[0]+x[2], x[1]+x[3]] for x in dctFiles[k]['xywhs']]
            lstRet.append({
                'filename' : k,
                'xyxys' : xyxys
            })
...
class MainAppLogic():
    def __init__(self, ui:pyqt5_ab_main.Ui_MainWindow, mainWindow):
        self.ui = ui
        self.strDSFolder = ''
        ...
        self.strDSType = ''
        self._UpdateDatasetFolder( 'q:/datasets/wider_face/out_train_multi')
    ...
    def _UpdateDatasetFolder(self, dir_choose):
        nonFixedImgSizes = ['widerface_train', 'widerface_val']
        try:
            strDsType = uiMain.cmbDSType.currentText().lower()
            if strDsType == 'subdataset':
                with open("%s/bboxes.json" % (dir_choose)) as fd:
                    self.lstGTs = json.load(fd)
            elif strDsType.find('widerface_') == 0:
                self.lstGTs = DSUtils.ParseWiderFace(dir_choose, strDsType[10:], uiMain.statusbar)
            self.strDSFolder = dir_choose
            self.strDSType = strDsType
            self.rndNdx = 0
            uiMain.isFixedImgSize = False if strDsType in nonFixedImgSizes else True
        except:
            uiMain.statusbar.showMessage('无效的数据集文件夹%s' % (dir_choose))
            self.rndNdx = -1
        
        MainWindow.setWindowTitle(_translate("MainWindow", "%s - 数据集目录：%s" % (self.appName, self.strDSFolder)))        
    ...
    def ShowValidate(self, ndx, strDSFolder = '', isShow = True):
        ...
        if self.strDSType == 'subdataset':
            filePart =  path.split(item['filename'])[1]
        else:
            filePart = item['filename']
...
class FraAB():
    def __init__(self, inWhich:QtWidgets.QGroupBox, ndx=0, w=191, h=135):
        '''
            生成一个配置锚框的frame
        '''
        ...
        _translate = QtCore.QCoreApplication.translate
        self.fraAB.setTitle(_translate("MainWindow", "下采样/%d" % (2**(ndx + 3))))
...
class MyMainWindow(pyqt5_ab_main.Ui_MainWindow):
    def __init__(self, hostWnd):
        ...
        self.cmbDSType.addItems(['VOC', 'COCO', 'subdataset', 'WiderFace_train', 'WiderFace_val'])
    ...
    def GetABConfig(self, isMakeABs=True, isVisualize=True) -> list:
        ...
        self.statusbar.showMessage('已贯彻新的锚框配置！', 5000)
        return 0
...
if __name__ == '__main__':
    ...
    # 创建窗口控件中的对象
    uiMain = MyMainWindow(MainWindow)    
    MainWindow.show()
